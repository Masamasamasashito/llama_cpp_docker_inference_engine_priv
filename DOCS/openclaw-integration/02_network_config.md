...
| docker-compose | ポート | GPU | コンテキスト | 並列 | スレッド | バッチ | メモリ制限 | 用途 |
|---|---|---|---|---|---|---|---|---|
| `docker-compose.yml` | 8081 | 99層 | 32768 | 1 | 8 | — | — | 標準GPU推論 |
| `docker-compose.cpu.yml` | 8082 | — | 2048 | 1 | — | — | 8GB上限/4GB予約 | GPU不要の検証用 |
| `docker-compose.high.yml` | 8083 | 99層 | 65536 | 1 | 24 | 4096/2048 | — | RTX 5090最適化 |
| `docker-compose.agents.yml` | 8084 | 99層 | 16384 | 1 | 8 | — | — | OpenClaw向け |

> **GPU版 vs High版の違い:** High版はRTX 5090 (32GB VRAM) 向けにコンテキスト2倍(65536)、バッチ処理(4096/2048)、スレッド3倍(24)に強化。
> **agents版:** OpenClawエージェント向け構成。
> **CPU版:** GPU不使用。メモリ8GB上限で軽量動作。検証・テスト用途。
> **並列(parallel):** 全構成でデフォルト1。司令塔が順次リクエストを振り分ける運用を前提とする。
